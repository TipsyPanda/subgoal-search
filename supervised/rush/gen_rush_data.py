...
# from gym_rush.envs.converter import CubeConverter
...
def random_move():
    ...
    return rush_solver_moves.Move(sample)
...
def get_raw_solution(string_observation, solver):
    if solver == 'Random':
        ...
        return rush_solver_utils.solve(string_observation, solver)
...
def generate_subgoal_learning_data(num_data):
    ...
    env = make_env_Rush(step_limit=100, shuffles=shuffles, obs_type=observation_type)
    ...
    while True:
        ...
        # print(rush_solver.solve(cube_bin_to_str(obs), 'Kociemba'))
...
def policy_validation(num_data, dist, model_move, final=False, brutal=False):
    env = make_env_Rush(step_limit=100, shuffles=0 if final else 60, obs_type=observation_type)
    face_tokens, move_tokens, col_to_id, move_token_to_id = policy_encoding()
...
def dist_solver_string(state, force_solver=None):
    ...
    try:
        solution = normalize_sequence(rush_solver_utils.solve(state, solver))
